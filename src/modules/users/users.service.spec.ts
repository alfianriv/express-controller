...
describe('UserService', () => {
    ...
    describe('create', () => {
        it('should create a user if username is unique', async () => {
            ...
            expect(userRepository.findOneByUsername).toHaveBeenCalledWith(dto.username);
        ...
        it('should throw BadRequestError if user already exists', async () => {
            ...
            vi.mocked(userRepository.findOneByUsername).mockResolvedValue(existingUser);
    ...
    describe('findOne', () => {
        it('should return a user by id', async () => {
            ...
            expect(userRepository.findOneById).toHaveBeenCalledWith(1, { validate: true });
            expect(result).toEqual(mockUser);
        });
    });
    ...
    describe('update', () => {
        ...
        it('shound not update throw error username already exist', async () => {
            ...
            vi.mocked(userRepository.findOneById).mockResolvedValue(mockUser);
            vi.mocked(userRepository.findOneByUsername).mockResolvedValue(createMockUser({ id: 2, username: 'test' }));
            vi.mocked(userRepository.update).mockResolvedValue(updatedUser);

            await expect(service.update(1, updateData)).rejects.toThrow(BadRequestError);
        });
        ...
        it('should update a user with new unique username', async () => {
            ...
            expect(userRepository.findOneByUsername).toHaveBeenCalledWith('newunique', { exceptionId: 1 });
            expect(userRepository.update).toHaveBeenCalledWith(1, updateData);
            expect(result).toEqual(updatedUser);
        });
    });
