...
describe('UserRepository', () => {
    ...
    describe('findOneById', () => {
        ...
        it('should throw NotFoundError if user not found (validate: true)', () => {
            expect(() => repository.findOneById(999, { validate: true })).toThrow(NotFoundError);
        });
    });
    ...
    describe('findOneByUsername', () => {
        it('should return a user by username', () => {
            const user = repository.createUser({ name: 'Test', username: 'test' });
            const result = repository.findOneByUsername('test', { exceptionId: user.id });
            expect(result).toEqual(user);
        });
        ...
        it('should throw NotFoundError if user not found (validate: true)', () => {
            expect(() => repository.findOneByUsername('unknown', { validate: true })).toThrow(NotFoundError);
        });

        it('should throw BadRequestError if username is taken by another user', () => {
            repository.createUser({ name: 'User 1', username: 'test' });
            expect(() => repository.findOneByUsername('test', { exceptionId: 999 })).toThrow(BadRequestError);
        });
    ...
    describe('createUser', () => {
        ...
        it('should throw Error if user with same id already exists', () => {
            const user = repository.createUser({ name: 'Test', username: 'test' });
            expect(() => repository.createUser({ id: user.id, name: 'Test 2' })).toThrow('User already exists');
        });
